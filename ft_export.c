...
/*   Updated: 2024/05/08 18:43:15 by mmad             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_buffer_to_list(t_list **head, char **command) // hena can not cause leaks
{
	int	i;
...
void	ft_buffer_to_list_v1(t_list **head, t_list* command) // hena can not cause leaks
{
...
int	ft_strchr_edit(const char *s, int c) // same function exit | hena can not cause leaks
{
	size_t i;
...
void	ft_link_node(t_list *head) // hena can not cause leaks
{
	...
	while (temp)
	{
		...
		{
			temp->key = ft_substr((char *)temp->content, 0, ft_strlen((char *)temp->content));
...
int	ft_search_if_key_exist(t_list **env, t_list *head) // hena can not cause leaks
{
	t_list	*temp_env;
